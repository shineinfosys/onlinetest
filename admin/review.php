...
<div class="container">
	<div class="panel panel-default">
	  <div class="panel-body">
	  		<h2 class='text-center'><span class="badge badge-light">Review Exam of </span><span class="badge badge-secondary" id="test_name"></span> - <span class="badge badge-light" id="username"></span></h2>
...
if(isset($_GET['unique_exam_key'])){
    ...
    $ques_query = "select mst_user.username, mst_useranswer.test_id, mst_useranswer.qid, mst_test.test_name, mst_question.que_desc, mst_question.ans1, mst_question.ans2, mst_question.ans3, mst_question.ans4, mst_question.true_ans, mst_useranswer.your_ans, mst_useranswer.time, mst_useranswer.unique_exam_key 
from mst_useranswer 
left join mst_test on mst_useranswer.test_id = mst_test.test_id
left join mst_question on mst_question.que_id = mst_useranswer.qid
left join mst_user on mst_useranswer.user_id = mst_user.user_id
...
order by mst_useranswer.qid, mst_useranswer.time asc";
    ...
    echo 'Direct access to review page not allowed! or invalid redirection.<br>Please contact to administrator.';
    exit;
}
...
while($row = mysqli_fetch_assoc($ques_result)) { 
	...
	$unique_exam_key = $row['unique_exam_key'];
	...
	if($counter == 1){
	    echo '<script>';
	    echo "$('#test_name').text('".$test_name."');";
	    echo "$('#username').text('".ucwords($username)."');";
	    echo '</script>';
	}
	...
	else if($counter == $rowcount)
	{
		...
		echo '							</li>';
		echo '							<li class="list-inline-item">';
		?>								<a name="submit_paper" href="<?php echo $_SERVER['HTTP_REFERER']; ?>" class="btn btn-primary btn-info-full next-step">Back to Previous Page</a>
		<?php
		echo '							</li>';
		echo '						</ul>';
	}
